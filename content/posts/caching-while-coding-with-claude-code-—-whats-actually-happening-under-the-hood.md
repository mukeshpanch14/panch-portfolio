...
Date: 2026-03-29
Category: Technology
Tags: claude, ai, claude-code, caching, developer-tools, agentic-coding, tokens
...
**3. File contents** — any source files Claude has read during the session. Once Claude reads your `package.json` or a component file, that content becomes part of the stable context prefix.

**4. Your new message** — what you just typed. Often 5–50 tokens.

The first three layers are what the cache stores. Your message is always fresh.
...
Cache reads are roughly **10× cheaper** than processing the same tokens fresh.
...
All of that happens automatically. CLAUDE.md would just add a project-specific prefix that persists *across* sessions too — useful, but not the primary driver of what you're seeing.
...
This usually means one of: you're starting lots of short sessions (cold starts dominate), your context is changing rapidly within sessions (lots of file switching, scope changes), or you're hitting the cache TTL between turns in a slow-moving session. The fix is usually longer sessions with tighter scope.
...
A healthy session looks like a smooth upward slope — cache reads growing steadily as context accumulates. A session in trouble looks like a flat line that never climbs, or a line that plateaus early and stops growing. Flat means Claude isn't reading files (pure conversation, not much work being done). Early plateau often means the context window compacted and you're in a loop.
